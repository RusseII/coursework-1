...
int main(int argc, char *argv[])
{
	...
	// setup the cities array and distances matrix. We will also setup the
	{
		...
		data_file = fopen("nqmqSmall.dat", "r");
                fscanf(data_file, "%d", &cities);
                char *whatwhatwhat[cities][200];
		...
		//cities = malloc(sizeof(char*) * num_cities);
	        for(i = 0; i <= cities; i++) {
                    fgets(buff, 1024, (FILE*)data_file);
                    if (i != 0){
                        strcpy(whatwhatwhat[i],buff);
                    }
                }
                fclose(data_file);

                //for(i=1; i <= cities; i++){
                //    printf("%s", whatwhatwhat[i]);
                //}
		...
		//int mem_size = num_cities * sizeof(int*) + num_cities *num_cities * sizeof(int);
                //

                //printf("doest his one print");
